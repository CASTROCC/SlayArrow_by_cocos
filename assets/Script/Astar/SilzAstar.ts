...
export class SilzAstar {
     ...
     */
    public find(xnow:number , ynow:number , xpos:number , ypos:number ) {
        xpos = Math.floor(xpos / MapConfig.GirdWidth);
        ypos = Math.floor(ypos / MapConfig.GirdHeight);
        xpos = Math.min(xpos, this._grid.numCols - 1);
        ypos = Math.min(ypos, this._grid.numRows - 1);
        this._grid.setEndNode(xpos, ypos); //1

        // xnow = Math.floor(xnow / MapConfig.GirdWidth);
        // ynow = Math.floor(ynow / MapConfig.GirdHeight);
        this._grid.setStartNode(xnow, ynow); //2

        var time: number = GameUtils.GetTime();
     ...
     * 当前格子是否可以通行
     */
    private isGirdNotCross(type: any): boolean {
        return type === Tile.blank 
            || type === Tile.wall 
            // || this._GirdType === Tile.door;
    }
    
    private makeGrid(mapData:number[][]) {
        var rows:number = mapData.length;
        var cols:number = mapData[0].length ;
        ...
        for(py = 0 ; py < rows ; py ++) {
            for(px = 0 ; px < cols ; px ++) {
                this._grid.setWalkable( px , py , !this.isGirdNotCross((<any>mapData[py][px]).type));
            }
        }
...
class AStar {
    ...
    private floydCrossAble(n1: PathNode, n2: PathNode): boolean {
        var ps: cc.Vec2[] = this.bresenhamNodes(cc.v2(n1.x, n1.y), cc.v2(n2.x, n2.y));
    ...
    private bresenhamNodes(p1: cc.Vec2, p2: cc.Vec2): cc.Vec2[] {
        ...
        if (steep) {
            ret.push(cc.v2(p1.y, p1.x));
        } else {
            ret.push(cc.v2(p1.x, p1.y));
        }
        while (nowX != p2.x) {
            var fy: number = Math.floor(nowY)
            var cy: number = Math.ceil(nowY);
            if (steep) {
                ret.push(cc.v2(fy, nowX));
            } else {
                ret.push(cc.v2(nowX, fy));
            }
            if (fy != cy) {
                if (steep) {
                    ret.push(cc.v2(cy, nowX));
                } else {
                    ret.push(cc.v2(nowX, cy));
                }
            }
            nowX += stepX;
            nowY += deltay;
        ...
        if (steep) {
            ret.push(cc.v2(p2.y, p2.x));
        } else {
            ret.push(cc.v2(p2.x, p2.y));
        }
        return ret;
    }
